...
| `magnus-publish` | Validate, build, commit to `staging`, and push. Reads `.magnus-changes/pending.log` to embed every gated approval into the commit message, then truncates the log. CI auto-deploys `staging.magnusconsulting.co.uk` on every successful push. Refuses to operate on any branch other than `staging`. |
...
| `magnus_validate` | Run the magnus repo's production build (and `astro check` if installed) on the user's Mac via the helper. **Required for Cowork** — the sandbox is Linux ARM64 and can't execute Mac-installed `node_modules` native binaries (rollup, sharp), so `npm run build` from inside Cowork always fails. This tool runs the same commands on the Mac and returns `{ passed, build, astroCheck }`. `magnus-publish` calls this as the pre-flight gate. |
